...
<template>
    <div
        class="request-mthod"
        v-loading="loading"
        element-loading-text="eth_getBlockByNumber Loading..."
        element-loading-background="rgba(122, 122, 122, 0.8)"
    >
        <h3 v-if="!provider">provider为空，请刷新页面重试</h3>
        <h2 class="title">eth_getBlockByNumber</h2>
        <div class="summary">通过数字返回有关块的信息</div>
...
const runProvider = () => {
    ...
    provider.value
        .request({
            ...
            params: [form.value.block, form.value.include],
        })
        .then((res) => {
