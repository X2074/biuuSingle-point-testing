...
<template>
    <div
        class="request-mthod"
        v-loading="loading"
        element-loading-text="eth_getTransactionCount Loading..."
        element-loading-background="rgba(122, 122, 122, 0.8)"
    >
        <h3 v-if="!provider">provider为空，请刷新页面重试</h3>
        <h2 class="title">eth_getTransactionCount</h2>
        <div class="summary">返回从某个地址发送的交易数量。</div>
...
const runProvider = () => {
    ...
    provider.value
        .request({
            ...
            params: [form.value.address, form.value.block],
        })
        .then((res) => {
